...
def rnn_conv(name, inputs, hiddens, filters, kernel_size, strides):
    ...
    with tf.compat.v1.variable_scope(name, reuse=tf.compat.v1.AUTO_REUSE):
        conv_inputs = tf.compat.v1.layers.conv2d(inputs=inputs, filters=gates_filters,
                                       kernel_size=kernel_size, strides=strides, padding='same', name='conv_inputs')
        conv_hidden = tf.compat.v1.layers.conv2d(inputs=hidden, filters=gates_filters,
...
class encoder(object):
    ...
    def encode(self, inputs):
        ...
        with tf.compat.v1.variable_scope('encoder', reuse=tf.compat.v1.AUTO_REUSE):
            encoder_rnn_input = tf.compat.v1.layers.conv2d(
    ...
    def binarizer(self, inputs, filters=32, kernel_size=(1, 1)):
        with tf.compat.v1.variable_scope('binarizer', reuse=tf.compat.v1.AUTO_REUSE):
            binarizer_input = tf.compat.v1.layers.conv2d(inputs=inputs, filters=filters,
...
class decoder(object):
    ...
    def decode(self, inputs):
        with tf.compat.v1.variable_scope('decoder', reuse=tf.compat.v1.AUTO_REUSE):
            decoder_rnn_input = tf.compat.v1.layers.conv2d(inputs=inputs, filters=DECODER_INPUT_DIM, kernel_size=[
            ...
            d_rnn_h1 = tf.compat.v1.depth_to_space(self.hiddens1[0], 2)
            self.hiddens2 = rnn_conv('decoder_rnn_conv_2',
                                     d_rnn_h1, self.hiddens2, DECODER2_DIM, [3, 3], (1, 1))
            d_rnn_h2 = tf.compat.v1.depth_to_space(self.hiddens2[0], 2)
            self.hiddens3 = rnn_conv('decoder_rnn_conv_3',
                                     d_rnn_h2, self.hiddens3, DECODER3_DIM, [3, 3], (1, 1))
            d_rnn_h3 = tf.compat.v1.depth_to_space(self.hiddens3[0], 2)
            self.hiddens4 = rnn_conv('decoder_rnn_conv_4',
                                     d_rnn_h3, self.hiddens4, DECODER4_DIM, [3, 3], (1, 1))
            d_rnn_h4 = tf.compat.v1.depth_to_space(self.hiddens4[0], 2)

            output = tf.compat.v1.layers.conv2d(inputs=d_rnn_h4, filters=3, kernel_size=[
